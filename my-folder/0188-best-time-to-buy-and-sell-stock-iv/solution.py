class Solution:
    def maxProfit(self, k: int, prices: List[int]) -> int:
        if k == 0: 
            return 0 # no transaction, hence no profit
        # dp[k][0] = min cost you need to spend at most k transactions
        # dp[k][1] = max cost you need to acheive at most k transactions
        dp = [[1000, 0] for _ in range(k+1)]
        for price in prices:
            for i in range(1, k+1):
                # price - dp[i - 1][1] is how much you need to spend
                # i.e use the profit earned from previous transaction to buy the stock
